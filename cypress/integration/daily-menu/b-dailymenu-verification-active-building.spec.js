...
let email = "byobAutomation7285b928-135d-4657-9421-e2e57273fe88@yopmail.net"
let deliverAtHome = 1
let categoryId = '2'
let orderQuantity = 2
let branchCapacity = 6

skipOn(Cypress.config().baseUrl.includes('ksa'), () => {

    describe.only('Daily Menu Verification of Active Building-One Restaurant', function () {
        before(function () {
            // CurrentDate
        ...
        it("C134975 Lunch Home Menu Verification of Active Building-One Restaurant", function () {
            this.test.testcaseId = 134975
            ...
            cy.getLocalStorage("token").then(userToken => {
                ...
                cy.getuserInformation(this.token).then(returnedResponse => {
                    ...
                    cy.dailyMenu(this.token, this.userId, deliverAtHome, categoryId, this.currentDate).then(returnedMenu => {
                        ...
                        // expect(returnedMenu.body.menus[0].deliversBy).to.be.an('string')
                        // expect(returnedMenu.body.menus[0].branchId).to.be.an('number')
                        // expect(returnedMenu.body.menus[0].userDeliveryTime).to.be.an('string')
                        // expect(returnedMenu.body.menus[0].cutOffColorId).to.be.an('number')
